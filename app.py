...
def init_db():
    ...
    try:
        ...
        # ensure referral codes (simple approach)
        cur.execute("SELECT id FROM users WHERE referral_code IS NULL OR referral_code=''")
        for row in cur.fetchall():
            cur.execute("UPDATE users SET referral_code=%s WHERE id=%s", (generate_referral_code(), row[0]))

        # create accounts for legacy users without accounts
        cur.execute("""
        ...
        for row in cur.fetchall():
            cur.execute(
                ...
                """,
                (row[0], STARTING_DEPOSIT_BALANCE),
            )
    ...
    if request.method == "POST":
        ...
        try:
            cur.execute(
                """
                INSERT INTO users (
                  username,fullname,phone,password_hash,withdraw_password_hash,password,withdraw_password,referral_code,referred_by
            ...
            cur.execute("INSERT INTO accounts (user_id,deposit_account) VALUES (%s,%s)", (user_id, STARTING_DEPOSIT_BALANCE))
            conn.commit()
        except Exception:
            conn.rollback()
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            if pw_hash:
                ...
                # fallback to legacy plaintext column if present
                legacy_pw = user.get("password")
                if legacy_pw is not None:
                    valid = legacy_pw == password
...
# ============================================================
# DASHBOARD / PLANS / PURCHASE / CLAIM
# ============================================================
...
def my_plan():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not plan:
                conn.rollback()
                flash("You do not have an active plan.", "error")
                return redirect(url_for("my_plan"))
            now = utcnow()
            ...
            if now >= end_time:
                ...
                conn.commit()
                flash("Your plan has completed its duration.", "error")
                return redirect(url_for("my_plan"))
            if now < next_claim:
                remaining = max(0, int((next_claim - now).total_seconds()))
                hours, rem = divmod(remaining, 3600)
                minutes, _ = divmod(rem, 60)
                conn.rollback()
                flash(f"Your next income can be claimed in {hours}h {minutes}m.", "error")
                return redirect(url_for("my_plan"))
    ...
    if plan:
        now = utcnow()
        end_time, nextc = plan_times(plan, now)
        if now >= end_time:
            ...
            seconds_remaining = max(0, int((nextc - now).total_seconds()))
            if now >= nextc:
                can_claim = True
                seconds_remaining = 0
            else:
                next_claim_timestamp = int(nextc.timestamp())
                next_claim = nextc
...
def deposit():
    ...
    if request.method == "POST":
        ...
        mime_type = next((m for ext, m in allowed.items() if filename.endswith(ext)), None)
        if not mime_type:
...
def request_withdrawal():
    ...
    if not user.get("withdraw_password_hash") and not user.get("withdraw_password"):
        flash("Withdrawal password is not configured.", "error")
        return redirect(url_for("withdraw"))

    # Check hashed-first then plaintext fallback
    try:
        valid = False
...
def change_login_password():
    ...
    if request.method == "POST":
        ...
        execute("UPDATE users SET password_hash=%s, password=%s WHERE id=%s", (new_hash, new_password, user["id"]))
...
def change_withdraw_password():
    ...
    if request.method == "POST":
        ...
        new_hash = generate_password_hash(new_password)
        execute("UPDATE users SET withdraw_password_hash=%s, withdraw_password=%s WHERE id=%s", (new_hash, new_password, user["id"]))
...
def admin_login():
    # Note: per request, no success flash on admin login.
    if request.method == "POST":
        username = request.form.get("username", "").strip()
        password = request.form.get("password", "")
    ...
    if request.method == "POST":
        ...
        if action in balance_actions:
            ...
            try:
                ...
                cur.execute("INSERT INTO transactions (user_id,transaction_type,amount,status,reference,description) VALUES (%s,'admin_balance_adjustment',%s,'successful',%s,%s)", (user_id, amount, generate_reference("ADM"), f"Admin adjustment: {action}"))
                conn.commit()
            except Exception:
                conn.rollback()
                app.logger.exception("ADMIN BALANCE ACTION FAILED")
        ...
        if action == "change_login_password":
            ...
            if len(new_password) < 6:
                ...
                new_hash = generate_password_hash(new_password)
                # Update both hashed and legacy plaintext for compatibility
                execute("UPDATE users SET password_hash=%s, password=%s WHERE id=%s", (new_hash, new_password, user_id))
                app.logger.info("Admin %s set login password for user %s", session.get("admin_id"), user_id)
        ...
        if action == "change_withdraw_password":
            ...
            if len(new_password) < 4:
                ...
                new_hash = generate_password_hash(new_password)
                execute("UPDATE users SET withdraw_password_hash=%s, withdraw_password=%s WHERE id=%s", (new_hash, new_password, user_id))
                app.logger.info("Admin %s set withdraw password for user %s", session.get("admin_id"), user_id)
...
# ============================================================
# ADMIN: DEPOSITS / WITHDRAWALS / INVITES
# ============================================================
...
def admin_deposit_action(deposit_id, action):
    ...
    try:
        ...
        conn.rollback()
        app.logger.exception("ADMIN DEPOSIT ACTION ERROR")
...
def admin_withdraw_action(withdrawal_id, action):
    ...
    try:
        ...
        if action == "approve":
            ...
            cur.execute("UPDATE transactions SET status='successful' WHERE id=(SELECT id FROM transactions WHERE user_id=%s AND transaction_type='withdrawal' AND amount=%s AND status='pending' ORDER BY created_at DESC LIMIT 1)", (user_id, amount))
            ...
            cur.execute("UPDATE transactions SET status='failed' WHERE id=(SELECT id FROM transactions WHERE user_id=%s AND transaction_type='withdrawal' AND amount=%s AND status='pending' ORDER BY created_at DESC LIMIT 1)", (user_id, amount))
            message = "Withdrawal rejected and balance restored."
        conn.commit()
        flash(message, "success")
    except Exception:
        conn.rollback()
        app.logger.exception("ADMIN WITHDRAWAL ACTION ERROR")
